...
const OtherQueriesScreen = () => {
  ...
  const handleSend = async() => {
    try {
      const FeatureRequestCollection = collection(db, 'FeatureRequest'); 
      await addDoc(FeatureRequestCollection, {
        query: query,
        createdAt: serverTimestamp(), 
      });
      console.log('Query sent:', query);
      Alert.alert('Success', 'Your Feature request has been sent!');
      setQuery(''); 
    } catch (error) {
    ...
    ...Platform.select({
      android: {
        paddingHorizontal: 80,
         // Adjust marginBottom for Android
      },
    }),
